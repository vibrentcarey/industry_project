...
const AvatarCard = ({ fname, lname, title }: Props) => {
	// Mode State
	...
	const [form, setForm] = useState({
		...
		about:
			"Enter a little something about your business to tell customers about yourself",
	...
	const handleAbout = (e: React.SyntheticEvent<HTMLTextAreaElement>) => {
	...
	return (
		<div className="w-full bg-backgroundGray p-6 rounded-md mt-10 mb-10 flex flex-col items-center shadow-lg">
			<span
				className="self-end text-xs mx-1 text-primaryBlue hover:cursor-pointer"
				onClick={handleEdit}
			>
				{editMode ? "save" : "edit"}
			</span>
			...
			{editMode && (
				<form className="flex flex-col w-full mt-4">
					...
					<input
						className="mt-1 border-b-2 border-black bg-backgroundGray"
						readOnly
						value={form.fullName}
					/>
					{/* Company */}
					...
					<textarea
						className="mt-1 border-2 rounded-md h-28 p-2 text-sm border-black bg-backgroundGray"
					...
					<input
						className="mt-1 mb-6 border-b-2 border-black bg-backgroundGray"
						readOnly
						value={form.address}
					/>
					{/* Website */}
